# Awesome IMAP MCP — Design Specification
...
- `docs/FEATURE_MATRIX.md` — the feature comparison remains valid, but the "Language" row for Awesome IMAP MCP should read "C# (.NET 8+)" not "TypeScript (Node)".
...
awesome-imap-mcp/
  AwesomeImapMcp.sln
  src/
    AwesomeImapMcp.Core/              # Shared types, config, DB, encryption, metrics
    AwesomeImapMcp.ImapClient/        # IMAP/SMTP abstraction, provider profiles, sync
    AwesomeImapMcp.Queue/             # Operation queue, worker, operation executors
    AwesomeImapMcp.Llm/               # LLM analysis pipeline (API, ACP, in-context)
    AwesomeImapMcp.McpServer/         # MCP tool definitions, server entry point, host
    AwesomeImapMcp.Dashboard/         # ASP.NET Core API + SignalR + React SPA
  tests/
    AwesomeImapMcp.Core.Tests/
    AwesomeImapMcp.ImapClient.Tests/
    AwesomeImapMcp.Queue.Tests/
    AwesomeImapMcp.Llm.Tests/
    AwesomeImapMcp.McpServer.Tests/
    AwesomeImapMcp.Dashboard.Tests/
    AwesomeImapMcp.IntegrationTests/  # Real IMAP server tests (Dovecot in Docker)
    AwesomeImapMcp.E2E.Tests/         # Full MCP flow + Dashboard Playwright tests
  dashboard/
...
1. Built-in defaults
2. `config.json` from default location (`~/.awesome-imap-mcp/config.json`)
3. `--config <path>` CLI argument
4. Environment variables (`AIMAP_SERVER_TRANSPORT=http`, etc.)
...
2. If not, derive from machine ID and warn: "No passphrase set — credentials are tied to this machine and are not portable. Set `AIMAP_PASSPHRASE` to make them portable."
...
{
    public static readonly ActivitySource ActivitySource = new("AwesomeImapMcp");
    public static readonly Meter Meter = new("AwesomeImapMcp");
...
- MSBuild target in `AwesomeImapMcp.Dashboard.csproj` runs `npm run build` and copies output to `wwwroot/`
...
2. **Global tool:** `dotnet tool install -g awesome-imap-mcp && awesome-imap-mcp`
3. **Container:** `docker run -v ~/.awesome-imap-mcp:/data awesome-imap-mcp`
4. **MCP client:** `claude mcp add awesome-imap-mcp -- awesome-imap-mcp`
