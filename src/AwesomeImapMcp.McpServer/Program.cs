...
using AwesomeImapMcp.Core.Logging;
...
using AwesomeImapMcp.RestBackend;
using AwesomeImapMcp.McpServer;
...
// Wire dashboard active status to coordinator
{
    ...
    var dashboardHost = host.Services.GetServices<IHostedService>()
        .OfType<AwesomeImapMcp.Dashboard.DashboardHost>().FirstOrDefault();
    ...
    List<AccountRecord> dbAccounts)
{
    var w = Console.Error;
    w.WriteLine();
    w.WriteLine("  Awesome IMAP MCP Server v0.1.0");
    w.WriteLine("  ================================");
    w.WriteLine();
    ...
    w.WriteLine("  Connect:");
    if (transport is "stdio" or "both")
        w.WriteLine("    claude mcp add awesome-imap-mcp -- awesome-imap-mcp");
    if (transport is "http" or "both")
    ...
    if (transport is "stdio")
    {
        ...
        w.WriteLine("    awesome-imap-mcp --transport both");
        w.WriteLine();
    }
}
