...
[McpServerToolType]
public class ReportTools(MessageRepository messageRepo, LlmAnalysisRepository analysisRepo, AppConfig config, ILogger<ReportTools> logger)
{
    ...
    {
        return McpJsonDefaults.LogToolCall(logger, "mailbox_report",
            new Dictionary<string, object?> { ["accountId"] = accountId, ["days"] = days },
            () =>
            {
                try
                {
                ...
                {
                    logger.LogError(ex, "MailboxReport failed");
                    return McpJsonDefaults.Error($"Report generation failed: {ex.Message}");
                }
            }, config);
    }
    ...
    {
        return McpJsonDefaults.LogToolCall(logger, "top_senders",
            new Dictionary<string, object?> { ["accountId"] = accountId, ["days"] = days, ["limit"] = limit },
            () =>
            {
                try
                {
                ...
                {
                    logger.LogError(ex, "TopSenders failed");
                    return McpJsonDefaults.Error($"Top senders query failed: {ex.Message}");
                }
            }, config);
    }
    ...
    {
        return McpJsonDefaults.LogToolCall(logger, "category_breakdown",
            ...
            () =>
            {
                ...
                {
                    logger.LogError(ex, "CategoryBreakdown failed");
                    return McpJsonDefaults.Error($"Category breakdown failed: {ex.Message}");
                }
            }, config);
    }
}
